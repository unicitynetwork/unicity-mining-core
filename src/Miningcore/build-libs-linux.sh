...
# Check if target already exists to avoid unnecessary rebuilds
if [ ! -f "$OutDir/librandomx.so" ]; then
    echo "Building librandomx.so..."
    ((cd /tmp && rm -rf RandomX && git clone https://github.com/unicitynetwork/RandomX && cd RandomX && mkdir build && cd build && cmake -DARCH=native .. && make) && (cd ../Native/librandomx && cp /tmp/RandomX/build/librandomx.a . && make clean && make) && mv ../Native/librandomx/librandomx.so "$OutDir")
else
    echo "librandomx.so already exists, skipping rebuild"
fi
